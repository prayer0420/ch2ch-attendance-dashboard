...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (typeof body.content !== "string" || !body.content.trim().length) {
      return NextResponse.json({ message: "게시할 내용을 입력해 주세요." }, { status: 400 });
    }

    const result = await publishToTargetBand(body.content.trim(), {
    ...
    return NextResponse.json({
      ...
      bandKey: result.band_key,
      bandUrl: `https://band.us/band/${encodeURIComponent(result.band_key)}`,
    });
  } catch (error) {
    return NextResponse.json(
