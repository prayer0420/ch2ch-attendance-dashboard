...
출석 Runner는 두 단계로 동작합니다.

1. `1-3부 참석` 또는 `4부 참석`인 사람만 웹교적 체크 대상이 됩니다. 참석하지 않은 사람과 `방송`만 체크된 사람은 출석 체크 경로에서 제외됩니다.
2. 출석 저장이 끝난 뒤 전체 명단을 읽기 전용으로 대조해, 시트 가족과 웹교적 소속이 다른 사람만 별도 결과와 로그로 남깁니다.

같은 Playwright 탭에서 화면 이동을 병렬 실행하면 가족 화면과 검색 화면이 서로 덮어쓸 수 있으므로, 실제 웹교적 조작은 순차 처리합니다. 검색 보정 재시도는 최대 2회입니다.
